    ...
    private ChatCompletionMessage merge(ChatCompletionMessage previous, ChatCompletionMessage current) {
        ...
        if (current.toolCalls() != null && !current.toolCalls().isEmpty()) {
            ...
            if (StringUtils.hasText(currentToolCall.id())) {
                // If the current tool call has the same ID as the last previous tool call, merge them
                if (lastPreviousTooCall != null && currentToolCall.id().equals(lastPreviousTooCall.id())) {
                    toolCalls.add(merge(lastPreviousTooCall, currentToolCall));
                } else {
                    // If the current tool call has a different ID, keep the last previous tool call and add the current one
